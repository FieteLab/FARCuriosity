...
def get_args(args_in=sys.argv[1:]):
    ...
    parser.add_argument('-curiosity_alg', type=str, choices=['none', 'icm', 'micm', 'disagreement', 'ndigo', 'rnd', 'frag_rnd'], help='Which intrinsic reward algorithm to use.')
    parser.add_argument('-env', type=str, help='Which environment to run on.')

    # general args
    parser.add_argument('-iterations', default=98304000, type=int, help='Number of optimization iterations to run (global timesteps).')
    ...
    parser.add_argument('-num_envs', default=32, type=int, help='Number of environments to run in parallel.')
    ...
    parser.add_argument('-num_cpus', default=16, type=int, help='Number of CPUs to run worker processes.')
    ...
    parser.add_argument('-timestep_limit', default=128, type=int, help='Max number of timesteps per trajectory')
    ...
    parser.add_argument('-record_freq', default=20, type=int, help='Interval between video recorded episodes (in episodes). 0 means dont record.')
    ...
    parser.add_argument('-prj_name', default='feat_curiosity', type=str, help='Directory where the results are stored. If None, use global.json[local_result_dir]')
    parser.add_argument('-entity', default=None, type=str, help='Directory where the results are stored. If None, use global.json[local_result_dir]')
    parser.add_argument('-max_decorrelation_steps', default=0, type=int, help='How many steps to run before the done flag is raised.')
    

    # FARCuriosity
    parser.add_argument('-fragmentation', action='store_true', help='Fragmentation')
    if '-fragmentation' in args_in:
        parser.add_argument('-recall', action='store_true', help='Use recall for fragmentation.')
        parser.add_argument('-frag_th', default=10.0, type=float, help='Fragmentation OOD threshold.')
        parser.add_argument('-recall_th', default=0.99, type=float, help='Recall threshold.')
        parser.add_argument('-cos_th', default=0.75, type=float, help='Cosine threshold for fragmentation.')
        parser.add_argument('-cos_th_min', default=0.5, type=float, help='Minimum cosine threshold for fragmentation.')
        parser.add_argument('-mem_size', default=800, type=int, help='Memory Size.')
        parser.add_argument('-use_feature', action='store_true', help='Use feature instead of observation for fragmentation and recall.')
        parser.add_argument('-frag_criteria', default='ratio', type=str, choices=['z', 'ratio'], help='Fragmentation criteria')
        parser.add_argument('-frag_obs_scale', default=1.0, type=float, help='Reduce the dimension of the fragmentation.')
    ...
    if environment in _ATARI_ENVS:
        parser.add_argument('-max_episode_steps', default=4500, type=int, help='How many steps to run before the done flag is raised.')
    ...
    elif 'MiniGrid' in environment:
        parser.add_argument('-max_episode_steps', default=500, type=int, help='How many steps to run before the done flag is raised.')
        parser.add_argument('-normalize_obs', action='store_true', help='Whether or not to normalize the observation each step.')
        parser.add_argument('-score_multiplier', default=1.0, type=float, help='A multiplier for the extrinsic reward.')
        parser.add_argument('-obs_type', default='rgb', type=str, choices=['mask', 'rgb', 'rgb_full'], help='Whether to pass binary mask observations or RGB observations.')
        parser.add_argument('-grayscale', action='store_true', help='Whether or not to grayscale images if using rgb.')
        parser.add_argument('-repeat_action_probability', default=0.0, type=float, help='Probability that an action will repeat (sticky actions).')
        parser.add_argument('-fire_on_reset', action='store_true', help='Whether or not to automatically press the fire button to start the game, or have the agent do this.')
        parser.add_argument('-fully_obs', action='store_true', help='Fully Observable State.')
    ...
    if curiosity_alg == 'icm':
        ...
        parser.add_argument('-curiosity_decay', action='store_true', help='Whether or not to linearly decay the curiosity signal')
        ...
        parser.add_argument('-prediction_beta', default=1.0, type=float, help='Scalar multiplier applied to the prediction error to generate the intrinsic reward. Environment dependent.')
        parser.add_argument('-drop_probability', default=0.25, type=float, help='Decimal percent of experience to drop when training the predictor model.')
        parser.add_argument('-dual_value', action='store_true', help='Whether or not to train separate value functions for extrinsic and intrinsic rewards.')
        parser.add_argument('-discount_ri', default=0.99, type=float, help='Sets intrinsic reward discounting. If dual value is not being used, make this equal to the extrinsic discount.')
        parser.add_argument('-curiosity_decay', action='store_true', help='Whether or not to linearly decay the curiosity signal')
    ...
    # logging 
    parser.add_argument('-eval_only', action='store_true', help='Evaluation Only flag.')
