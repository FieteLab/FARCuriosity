...
class RewardForwardFilter(object):
    def __init__(self, gamma, size=None):
        if size is None:
            self.rewems = None
        else:
            self.rewems = np.zeros(size)
        self.gamma = gamma

    def update(self, rews, not_done=None, update=None):
        if not_done is None:
            if self.rewems is None:
                if update is None:
                    self.rewems = rews
                else:
                    self.rewems[update] = rews[update]
            elif update is None:
                self.rewems = self.rewems * self.gamma + rews
            else:
                self.rewems[update] = self.rewems[update] * self.gamma + rews[update]
                return self.rewems[update]
            return self.rewems
            ...
            if self.rewems is None:
                ...
                mask = np.where(not_done[update] == 1.0)
                self.rewems[update][mask] = self.rewems[update][mask] * self.gamma + rews[update][mask]
                return deepcopy(self.rewems[update])
