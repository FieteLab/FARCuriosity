...
def buffer_from_example(example, leading_dims, share_memory=False,
    ...
    for v in example:
        if type(v) is not tuple:
            t = buffer_from_example(v, leading_dims, share_memory=share_memory, use_NatSchema=use_NatSchema)
            out.append(t)
        else:
            t = (buffer_from_example(vv, leading_dims, share_memory=share_memory, use_NatSchema=use_NatSchema) for vv in v)
            out.append(t)
    
    out2 = buffer_type(*out)
    return out2

    return buffer_type(*(buffer_from_example(v, leading_dims,
        share_memory=share_memory, use_NatSchema=use_NatsShema)
        for v in example))
