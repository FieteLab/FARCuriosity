...
from rlpyt.models.curiosity.fragmentation_wrapper import FragmentationWrapper
...
class AtariLstmModel(torch.nn.Module):
    ...
    def __init__(
            ...
            obs_stats=None,
            fragmentation_kwargs=dict(fragmentation=False),
            ):
        ...
        if curiosity_kwargs['curiosity_alg'] != 'none':
            ...
            if fragmentation_kwargs['fragmentation']:
                self.curiosity_model = FragmentationWrapper(curiosity_kwargs['curiosity_alg'],
                        image_shape, output_size, fragmentation_kwargs, curiosity_init_kwargs, self.obs_stats)
            elif curiosity_kwargs['curiosity_alg'] == 'icm':
                self.curiosity_model = ICM(image_shape=image_shape, action_size=output_size, **curiosity_init_kwargs)
            elif curiosity_kwargs['curiosity_alg'] == 'micm':
    ...
    def forward(self, image, prev_action, prev_reward, init_rnn_state):
        ...
        *image_shape], with T=1,B=1 when not given.  Expects uint8 images in
        ...
        return pi, pi_ext, pi_int, v, v_int, next_rnn_state
