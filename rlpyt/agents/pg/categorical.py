...
class RecurrentCategoricalPgAgentBase(BaseAgent):
    ...
    def __call__(self, observation, prev_action, prev_reward, init_rnn_state):
        ...
        pi, pi_ext, pi_int, value, value_int, next_rnn_state = self.model(*model_inputs)
        dist_info, dist_ext_info, dist_int_info, value, value_int = buffer_to((DistInfo(prob=pi), DistInfo(prob=pi_ext), DistInfo(prob=pi_int), value, value_int), device="cpu")
        return dist_info, dist_ext_info, dist_int_info, value, value_int, next_rnn_state  # Leave rnn_state on device.
    ...
    def step(self, observation, prev_action, prev_reward):
        ...
        pi, pi_ext, pi_int, value, value_int, rnn_state = self.model(*agent_inputs, self.prev_rnn_state)
        dist_info = DistInfo(prob=pi)
        ...
        agent_info = AgentInfoRnn(dist_info=dist_info, dist_ext_info=dist_ext_info, dist_int_info=dist_int_info, value=value, value_int=value_int, prev_rnn_state=prev_rnn_state)
        action, agent_info = buffer_to((action, agent_info), device="cpu")
    ...
    def curiosity_loss(self, curiosity_type, *args, indices=None):
        ...
        if curiosity_type in {'icm', 'micm'}:
            ...
            inv_loss, forward_loss = self.model.curiosity_model.compute_loss(*curiosity_agent_inputs, indices)
            ...
            forward_loss = self.model.curiosity_model.compute_loss(*curiosity_agent_inputs, indices)
            losses = (forward_loss.to("cpu"))
    ...
    def value(self, observation, prev_action, prev_reward):
        ...
        agent_inputs = buffer_to((observation, prev_action, prev_reward),
            device=self.device)
        _pi, _pi_ext, _pi_int, value, value_int, _rnn_state = self.model(*agent_inputs, self.prev_rnn_state)
        return value.to("cpu"), value_int.to("cpu")
