...
class MinibatchRlBase(BaseRunner):
    ...
    def shutdown(self):
        ...
        os.system("touch {}/done".format(self.log_dir))

    def get_itr_snapshot(self, itr):
        """
        Returns state needed for full checkpoint/snapshot of training run,
        ...
        return dict(
            ...
            algo_state_dict=self.algo.algo_state_dict(),
        )
    ...
    def log_diagnostics(self, itr, traj_infos=None, eval_time=0, prefix='Diagnostics/'):
        ...
        if itr > 0:
            self.pbar.stop()
        if itr >= self.min_itr_learn - 1 and itr % self.model_save_freq == 0:
    ...
    def _log_trajectory_stats(self, traj_infos=None):
        """
...
class MinibatchRl(MinibatchRlBase):
    ...
    def train(self):
        ...
        """
        n_itr = self.startup()
        if self.pretrain != 'None' and self.pretrain is not None:
