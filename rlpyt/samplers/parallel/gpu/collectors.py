    ...
    def collect_evaluation(self, itr):
        ...
        for t in range(self.max_T):
            ...
            if self.sync.stop_eval.value:
                ...
                break
            for b, env in enumerate(self.envs):
                o, r, d, env_info = env.step(step.prev_action[b])
                traj_infos[b].step(step.prev_observation[b], step.prev_action[b], r, d,
                    step.agent_info[b], env_info)
                if getattr(env_info, "traj_done", d):
                    self.traj_infos_queue.put(traj_infos[b].terminate())
                    traj_infos[b] = self.TrajInfoCls()
                    o = env.reset()
                step.prev_observation[b] = o
                step.prev_reward[b] = r
                step.done[b] = d
            obs_ready.release()
