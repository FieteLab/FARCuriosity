...
class ActionServer:
    ...
    def serve_actions(self, itr):
        ...
        if "bootstrap_value" in self.samples_np.agent:
            self.samples_np.agent.bootstrap_value[:], self.samples_np.agent.bootstrap_value_int[:] = self.agent.value(*agent_inputs)
    ...
    def serve_actions_evaluation(self, itr):
        ...
        traj_infos = list()
        self.agent.reset()
        agent_inputs = AgentInputs(step_pyt.prev_observation, step_pyt.prev_action,
            step_pyt.prev_reward)  # Fixed buffer objects.
        ...
        for t in range(self.eval_max_T):
            ...
            for b in obs_ready:
                ...
                # assert not b.acquire(block=False)  # Debug check.
            for b_reset in np.where(step_np.done)[0]:
                step_np.prev_action[b_reset] = 0  # Null prev_action.
                step_np.prev_reward[b_reset] = 0  # Null prev_reward.
                self.agent.reset_one(idx=b_reset)
            action, agent_info = self.agent.step(*agent_inputs)
            step_np.prev_action[:] = action
